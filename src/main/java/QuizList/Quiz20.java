...
// 해시사용방법
        ...
        { map.put(name, map.getOrDefault(name, 0) + 1); }  // 해쉬함수 디폴트값입력 0 여기서는 참가자는 +1

        for (String name : completion)
        {  map.put(name, map.get(name) - 1); }    // 갖고 있는 모든 키를 순회(keySet) 완주한자의 -1을 해준다 .

        for (String key : map.keySet())      // 모든 키를 불러온다.
        {  if (map.get(key) > 0)            // 키의 값이 0이면 완주, 0이 아니면 탈락자. 즉 1이라면 탈락자
            { answer = key;
                break;
            }
        }
